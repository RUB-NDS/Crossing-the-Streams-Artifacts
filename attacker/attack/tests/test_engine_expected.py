...
def test_expected_none_returns_none() -> None:
    assert _check_expected_match(b"a", 0, None) is None


def test_position_past_expected_returns_none() -> None:
...
def test_matching_byte_returns_none() -> None:
    assert _check_expected_match(b"b", 1, b"abc") is None


def test_mismatching_byte_returns_dict() -> None:
    info = _check_expected_match(b"x", 1, b"abc")
    assert info == {"expected_byte": "b", "committed_byte": "x"}


def test_mismatch_at_terminator_position_is_detected() -> None:
...
def test_mismatch_with_control_bytes() -> None:
