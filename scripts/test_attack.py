"""End-to-end attack test against several known passwords.
...
For each test secret it
  1. POSTs /set_secret to the client (which also reconnects SSH so the
     LZ77 dictionary starts fresh -- the attacker container never sees
     this call).
...
def http(method: str, url: str, body: bytes | None = None,
         content_type: str | None = None, timeout: float = 600.0) -> dict:
    headers = {}
    if content_type:
...
def main() -> int:
    print("CRIME-on-SSH PoC: end-to-end attack test")
    ...
    for idx, secret in enumerate(TEST_SECRETS, 1):
        print("=" * 72)
        print(f"  [{idx}/{len(TEST_SECRETS)}] target secret = {secret!r}")
        print("=" * 72)
        set_secret(secret)
        # let the client finish reconnecting
        time.sleep(1.0)
